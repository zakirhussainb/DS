...
public class Graph {
    ...
    public LinkedList<Vertex>[] adjListArrayForDirected;
    public Graph(int vertex) {
        this.vertex = vertex;
        adjListArray = new LinkedList[vertex];
        adjListArrayWithWeights = new LinkedList[vertex];
        adjListArrayForDirected = new LinkedList[vertex];
        for(int i = 0; i < vertex; i++) {
            adjListArray[i] = new LinkedList<>();
            adjListArrayWithWeights[i] = new LinkedList<>();
            adjListArrayForDirected[i] = new LinkedList<>();
        }
    }
    ...
    public void addEdgeWithNegativeWeight(int u, int v, int weight) {
        adjListArrayForDirected[u].add(new Vertex(v, weight));
    }
...
// The given Map stores the <current_vertex, {predecessor, distance}>
    ...
    public void printGraphWithWeights(LinkedList<Vertex>[] adjListArray) {
        for (int i = 0; i < vertex; i++) {
            System.out.println("\nAdjacency list of vertex " + i);
            System.out.print("head");
            for (int j = 0; j < adjListArray[i].size(); j++) {
