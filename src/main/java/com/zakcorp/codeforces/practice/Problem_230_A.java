...
public class Problem_230_A {
    ...
    static class Solver {
        public void solve1(InputReader in, OutputWriter out) {
            ...
            int def = 0;
            for(int i = 0; i < n; i++) {
                pq.add(new Pair(in.readInt(), in.readInt()));
            }
            while( !pq.isEmpty() ) {
                Pair p = pq.poll();
                if(s > p.ds) {
    ...
    static class MyComparator implements Comparator<Pair> {
        ...
        public int compare(Pair p1, Pair p2) {
            return Integer.compare(p1.ds, p2.ds);
        }
    }
